...
### adapted from https://github.com/omegafragger/DDU
...
from DDU.spectral_normalization.spectral_norm_conv_inplace import spectral_norm_conv
from DDU.spectral_normalization.spectral_norm_fc import spectral_norm_fc
...
class ResNet(nn.Module):
    ...
    def forward(self, x):
        ...
        self.feature = out.clone() #change: no detach
        out = self.fc(out) / self.temp
        return F.log_softmax(out, dim=1) #change: softmax added


def resnet18(spectral_normalization=True, mod=True, temp=1.0, mnist=True, **kwargs): #change: mnist=True
    model = ResNet(
        BasicBlock,
        [2, 2, 2, 2],
